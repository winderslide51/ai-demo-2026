...
2. `AGENTS.md`, puis `docs/SPEC-TECHNIQUE.md` en entier : tu conçois **depuis l'existant**,
   pas depuis une page blanche. Étendre vaut mieux qu'introduire.
...
2. **Règle métier.** Formule-la comme une fonction pure de `domain/` : signature, entrées,
   ...
   la main** — c'est ce que `test-dev` transformera en test.
